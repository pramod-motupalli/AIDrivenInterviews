...
export default function CandidateInterview() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (data.role !== 'candidate') { setLoginError('This portal is for candidates only.'); return; }

      setAccessToken(data.access);
      if (data.is_first_login) {
        setIsFirstLogin(true);   // show password reset screen
      } else {
        setPhase('intro');
        connectWs(data.access);
      }
    } catch {
  ...
  const handlePasswordReset = async (e) => {
    ...
    setLoginError('');
    try {
      const res = await fetch(`${API_BASE}/auth/password-reset/first-login/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${accessToken}` },
        body: JSON.stringify({ new_password: newPassword }),
      });
      const data = await res.json();
      if (!res.ok) { setLoginError(data.error || 'Failed to reset password.'); return; }
  ...
  const connectWs = (jwt) => {
    ...
    ws.onclose = () => { if (wsStatus !== 'completed') setWsStatus('error'); };
  };
        ...
        <div className="flex-1 flex flex-col max-w-3xl mx-auto w-full p-4 gap-4">
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-4 backdrop-blur">
            ...
            <div className="flex items-center justify-between mt-3">
              <p className="text-slate-500 text-xs">Ctrl + Enter to submit</p>
              <button
                onClick={submitAnswer}
                disabled={!answer.trim() || submitting || wsStatus !== 'active'}
                className="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-semibold transition-all disabled:opacity-40 flex items-center gap-2"
              >
                Submit
