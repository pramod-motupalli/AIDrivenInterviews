...
const Results = () => {
  ...
  useEffect(() => {
    ...
    interviewService.getResults(interviewId)
      .then(data => {
        // Normalize the backend response into the shape the UI expects
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#f8fafc', gap: 16 }}>
        ...
        <span style={{ fontWeight: 700, color: '#64748b', textTransform: 'uppercase', fontSize: 13, letterSpacing: '0.1em' }}>Loading Your Results...</span>
  ...
  if (error || !results) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#f8fafc', gap: 16, padding: 32 }}>
        <style>{rsStyles}</style>
        <div style={{ background: '#fff', border: '1px solid #fecdd3', borderRadius: 20, padding: 40, maxWidth: 480, textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: 20, fontWeight: 800, color: '#0f172a', marginBottom: 8 }}>Results Not Yet Available</h2>
          <p style={{ color: '#64748b', lineHeight: 1.6 }}>{error || 'Your results are still being processed by the AI. Please try again in a moment.'}</p>
        </div>
      </div>
    );
  }
