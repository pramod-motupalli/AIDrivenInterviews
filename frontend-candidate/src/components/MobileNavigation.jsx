...
const MobileNavigation = () => {
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-xl border-t border-slate-100 pb-safe pt-3 px-6 flex justify-between items-center z-50 shadow-[0_-8px_30px_rgba(0,0,0,0.04)]">
      {navItems.map((item) => {
        ...
        return (
          <button 
            ...
            className={`flex flex-col items-center gap-1.5 p-2 transition-all duration-300 min-w-[64px] ${active ? 'text-indigo-600' : 'text-slate-400'}`}
          >
